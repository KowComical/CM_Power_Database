...
file_path = 'K:\\Github\\GlobalPowerUpdate-Kow\\data\\#global_rf\\iea\\'
...
path = re.compile(r'<a class="a-link a-link--accent a-link--plain " href="(?P<path>.*?)"', re.S)
...
df_iea = df_iea[
    ['country', 'year', 'month', 'Total Combustible Fuels', 'coal', 'gas', 'oil', 'nuclear', 'hydro', 'solar', 'wind',
     'other']].reset_index(
    drop=True)
df_iea.sort_values(['country', 'year', 'month']).to_csv(os.path.join(file_path, 'iea_all.csv'), index=False,
                                                        encoding='utf_8_sig')
