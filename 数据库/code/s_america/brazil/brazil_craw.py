...
path = 'K:\\Github\\GlobalPowerUpdate-Kow\\数据库\\data\\s_america\\brazil\\raw'
...
def initialSession():
    url = 'https://tableau.ons.org.br/t/ONS_Publico/views/GeraodeEnergia/HistricoGeraodeEnergia?:embed=y&:showAppBanner=false&:showShareOptions=true&:display_count=no&:showVizHome=no'
    ...
    playload = {
        ...
        'stickySessionKey': '{"featureFlags":"{}","isAuthoring":false,"isOfflineMode":false,"lastUpdatedAt":1596748654166,"viewId":309,"workbookId":273}',
        'filterTileSize': 200,
        'locale': 'en_US',
        'language': 'en',
        'verboseMode': 'false',
    ...
    url = 'https://tableau.ons.org.br/vizql/t/ONS_Publico/w/GeraodeEnergia/v/HistricoGeraodeEnergia/bootstrapSession/sessions/%s' % sessionID
    res = requests.post(url, data=playload, verify=False)
    print('Initialize Session...')

    # Setting paramters
    url = 'https://tableau.ons.org.br/vizql/t/ONS_Publico/w/GeraodeEnergia/v/HistricoGeraodeEnergia/sessions/%s/commands/tabdoc/set-parameter-value' % sessionID
...
def setProductionType(sessionID, t):
    ...
    url = 'https://tableau.ons.org.br/vizql/t/ONS_Publico/w/GeraodeEnergia/v/HistricoGeraodeEnergia/sessions/%s/commands/tabdoc/categorical-filter-by-index' % sessionID
    fields = {
        'visualIdPresModel': '{"worksheet":"Simples Geração de Energia Barra Semana","dashboard":"Painel Simples Geração de Energia","storyboard":"Histórico Geração de Energia","storyPointId":5}',
        'globalFieldName': '[federated.14ob1hf1l48s251gwbqcg15ujjvd].[none:nom_tipousinasite, nom_tipousinasite, nom_tipousinasite and 1 more:nk]',
        'filterIndices': '[%d]' % t,
...
def setThermalType(sessionID, t):
    ...
    fields = {
        'visualIdPresModel': '{"worksheet":"Simples Geração de Energia Barra Semana","dashboard":"Painel Simples Geração de Energia","storyboard":"Histórico Geração de Energia","storyPointId":5}',
        'globalFieldName': '[federated.14ob1hf1l48s251gwbqcg15ujjvd].[none:nom_tipocombustivel (tb_referenciacegusina (Conjunto)), nom_tipocombustivel (tb_referenciacegusina1 (PrevCarga)), nom_tipocombustivel (tb_referenciacegusina (Pequena)) e 1 mais:nk]',
...
def resolveData(sessionID, t):
    ...
    url = 'https://tableau.ons.org.br/vizql/t/ONS_Publico/w/GeraodeEnergia/v/HistricoGeraodeEnergia/vudcsv/sessions/%s/views/%s?summary=true' % (
        sessionID, viewID)
